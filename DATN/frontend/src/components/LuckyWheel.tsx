...
const LuckyWheel: React.FC = () => {
  ...
  useEffect(() => {
    if (user?.email) {
      ...
      // Nếu người dùng đăng xuất, reset hasSpun cho UI của phiên hiện tại
      setHasSpun(false);
    }
  }, [user]);
  ...
  useEffect(() => {
    ...
    // Nếu người dùng đã quay vĩnh viễn, ẩn vòng quay
    if (hasSpun) {
      setOpen(false);
      return;
    }

    // Kiểm tra trạng thái đã đóng trong session (cho cả người dùng đã/chưa đăng nhập)
    const dismissedForCurrentLoginState = sessionStorage.getItem(`luckywheel_dismissed_${isLoggedIn ? 'logged_in' : 'logged_out'}_session`);

    if (!dismissedForCurrentLoginState) {
        setOpen(true);
    } else {
        setOpen(false);
    }
  }, [hasSpun, availableVouchers, isLoggedIn]);

  const handleClose = () => {
    setOpen(false);
    // Lưu trạng thái đã đóng vào sessionStorage cho trạng thái đăng nhập hiện tại
    sessionStorage.setItem(`luckywheel_dismissed_${isLoggedIn ? 'logged_in' : 'logged_out'}_session`, 'true');
  };
