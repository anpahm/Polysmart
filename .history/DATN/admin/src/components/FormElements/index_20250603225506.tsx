...
export default function ProductTable() {
  ...
  const handleShowVariants = async (productId: string) => {
    ...
    try {
      ...
      const data = await res.json();
      console.log('Variants data:', data); // Để debug
  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow-lg">
      ...
      {variantModal.open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm pl-50 pt-20">
          <div className="bg-white rounded-xl shadow-lg p-6 w-[1000px] relative">
            <button
                ...
                <table className="min-w-full border">
                  <thead>
                    <tr className="bg-gray-100">
                      ...
                      <th className="px-3 py-2 border">Phiên bản</th>
                      ...
                      <th className="px-3 py-2 border">Giá bán</th>
                        ...
                        <td className="px-3 py-2 border">
                          <div className="relative group">
                            <div className="relative w-16 h-16 transition-transform hover:scale-105">
                              ...
                              <div className="absolute inset-0 bg-black opacity-0 group-hover:opacity-20 transition-opacity rounded"></div>
                            </div>
                            {v.hinh && v.hinh.length > 1 && (
                              <span className="absolute -top-2 -right-2 bg-blue-500 text-white text-xs rounded-full w-6 h-6 flex items-center justify-center shadow-lg">
                                +{v.hinh.length - 1}
                              </span>
                            )}
                          </div>
                        </td>
                        <td className="px-3 py-2 border">{v.phien_ban || '-'}</td>
                        <td className="px-3 py-2 border">{v.dung_luong || '-'}</td>
                        <td className="px-3 py-2 border">{v.mau || '-'}</td>
                        <td className="px-3 py-2 border">{v.gia_goc ? v.gia_goc.toLocaleString('vi-VN') + '₫' : '-'}</td>
                        <td className="px-3 py-2 border">{v.gia ? v.gia.toLocaleString('vi-VN') + '₫' : '-'}</td>
                        <td className="px-3 py-2 border">{v.so_luong_hang ?? '-'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      {selectedImages && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/90">
          <div className="relative w-full h-full flex flex-col items-center justify-center p-4">
            {/* Close button */}
            <button
              className="absolute top-4 right-4 text-white hover:text-gray-300 text-xl z-50"
            ...
            <div className="relative w-full h-[70vh] max-w-4xl">
              <Image
                src={getImageUrl(selectedImages.images[selectedImages.currentIndex])}
            ...
            {/* Thumbnails */}
            <div className="flex gap-2 mt-4 overflow-x-auto max-w-4xl px-4">
              {selectedImages.images.map((img, idx) => (
                <div
                  key={idx}
                  className={`relative w-16 h-16 flex-shrink-0 transition-transform hover:scale-105 cursor-pointer
                    ${idx === selectedImages.currentIndex ? 'ring-2 ring-blue-500' : ''}`}
                  onClick={() => setSelectedImages(prev => ({ ...prev, currentIndex: idx }))}
                >
                  <Image
                    src={getImageUrl(img)}
                    alt={`Thumbnail ${idx + 1}`}
