...
const GiftVouchersPage = () => {
  ...
  const sortedVouchers = [...vouchers].sort((a, b) => new Date(b.ngay_tao).getTime() - new Date(a.ngay_tao).getTime());
          ...
          <table className="w-full table-auto">
            ...
            <tbody>
              {sortedVouchers.map((voucher, idx) => (
                <tr key={voucher._id} className={`${voucher.da_vo_hieu_hoa ? 'opacity-60' : ''}`}>
                  ...
                  <td className="border-b border-[#eee] py-4 px-4 dark:border-strokedark">
                    <div className="font-mono text-sm bg-gray-100 p-2 rounded-lg text-center">{voucher.ma_voucher}</div>
                  </td>
                  <td className="border-b border-[#eee] py-4 px-4 dark:border-strokedark text-center">
                    <span className={`inline-flex rounded-full py-1 px-3 text-sm font-bold ${voucher.phan_tram ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-400'}`}>{voucher.phan_tram ? voucher.phan_tram + '%' : '-'}</span>
                  </td>
                  <td className="border-b border-[#eee] py-4 px-4 dark:border-strokedark text-center">
                    <div className="flex flex-col gap-1 items-center">
                      <span className={`inline-flex rounded-full py-1 px-3 text-xs font-medium ${voucher.da_su_dung ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>{voucher.da_su_dung ? 'Đã sử dụng' : 'Chưa sử dụng'}</span>
                      <span className={`inline-flex rounded-full py-1 px-3 text-xs font-medium ${voucher.email_da_gui ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-600'}`}>{voucher.email_da_gui ? 'Đã gửi email' : 'Chưa gửi email'}</span>
                    </div>
                  </td>
                  <td className="border-b border-[#eee] py-4 px-4 dark:border-strokedark">
                    <div className="text-sm">
                      <p>{voucher.email}</p>
                      {voucher.email_gui_luc && (
                        <p className="text-xs text-gray-500">Gửi: {formatDate(voucher.email_gui_luc)}</p>
                      )}
                    </div>
                  </td>
                  <td className="border-b border-[#eee] py-4 px-4 dark:border-strokedark">
                    <div className="text-sm">
                      <p>{formatDate(voucher.ngay_tao)}</p>
                      <p className="text-xs text-gray-500">Hết hạn: {voucher.het_han ? new Date(voucher.het_han).toLocaleDateString('vi-VN') : '-'}</p>
                    </div>
                  </td>
                  <td className="border-b border-[#eee] py-4 px-4 dark:border-strokedark text-center">
                    <div className="flex items-center justify-center space-x-3.5">
                      {!voucher.da_vo_hieu_hoa && !voucher.email_da_gui && (
