  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch tất cả sản phẩm có khuyến mãi
        const flashSaleResponse = await fetch(getApiUrl('products'));
        const flashSaleData = await flashSaleResponse.json();
        
        // Lọc sản phẩm có khuyến mãi và sắp xếp theo % giảm giá
        const flashSaleProducts = Array.isArray(flashSaleData) 
          ? flashSaleData
              .filter(product => product.khuyen_mai && product.khuyen_mai > 0)
          ...
          <div className="bg-gradient-to-r from-red-600 to-pink-500 rounded-2xl p-8 shadow-xl overflow-hidden relative">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
              {data.flashSaleProducts.map((product) => {
                ...
                const discountPercent = product.khuyen_mai || 0;
                  ...
                  >
                    ...
                    <div className="p-4">
                      ...
                      <div className="flex flex-col gap-1">
                        ...
                        <div className="text-xs text-red-500">Tiết kiệm {formatCurrency(originalPrice - salePrice)}</div>
                      </div>
                    </div>
                  </Link>
                );
