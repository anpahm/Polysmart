...
const HomePage = () => {
  ...
  const handleFlashSaleClick = async (variant: FlashSaleVariantInHomepage) => {
    try {
      ...
      const url = `/product/${variant?.product_id || ""}?flashsale=${
        variant.id_variant
  ...
  const checkOrderStatus = async (orderId: string) => {
    try {
      ...
      if (
        ...
        orderData.items &&
        orderData.items.some((item: any) => item.isFlashSale)
      ) {
  ...
  const hotIphones: Product[] = data.iPhoneProducts.filter(
    (product: Product) => product.ban_chay > 10000
  );
                          ...
                          >
                            ...
                            <div className="relative pt-[100%] overflow-hidden">
                              ...
                              <Image
                                ...
                                alt={
                                  variant?.product_name || "Sản phẩm Flash Sale"
                                }
                                fill
          ...
          <section className="section bg-white">
            <div className="container mx-auto px-4 sm:px-6 md:px-10 lg:px-40">
              <div className="section-header flex justify-between items-center mb-6 bg-white">
                <PetMascot
                  message={aiAdvice || "Xin chào, đây là gợi ý cho bạn!"}
                />
              </div>
                                ...
                                <div className="flex flex-wrap gap-2 mb-2">
                                  {product.variants
                                    ...
                                    .map((variant, idx) => (
                                      <span
                                        key={idx}
                                        className="px-2 py-1 text-[10px] border border-gray-300 bg-gray-50 text-gray-700 rounded-full"
                                      >
                                        {variant.dung_luong}
                                      </span>
                                    ))}
