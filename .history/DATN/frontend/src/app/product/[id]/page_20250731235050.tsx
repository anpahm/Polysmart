...
<div className="flex flex-col lg:flex-row gap-6">
  ...
  <div className="flex flex-col items-center mt-6 lg:mt-0 lg:ml-8 w-full">
    ...
    <div className="text-sm text-gray-500 mb-4">
      Tiết kiệm: {(originPrice + selectedAccessories.reduce((sum, accId) => {
        const acc = accessories.find(a => a._id === accId);
        const g = acc?.gia ?? acc?.variants?.[0]?.gia ?? 0;
        return sum + g;
      }, 0) - calculateTotal()).toLocaleString()}₫
    </div>
    <button
      className="bg-blue-600 text-white w-full lg:w-auto px-6 py-3 rounded-lg font-bold text-lg hover:bg-blue-700 transition"
      onClick={...} // giữ nguyên handler như cũ
    >
