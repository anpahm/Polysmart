    ...
    <div className="p-6 bg-white rounded-lg shadow-lg">
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-lg p-8 w-full max-w-md relative max-h-[90vh] flex flex-col">
                  ...
                  onChange={async (e) => {
                    ...
                    for (let i = 0; i < files.length; i++) {
                      ...
                      try {
                        ...
                        if (!res.ok) throw new Error("Lỗi upload hình");
                        const data = await res.json();
                        if (data?.url) uploadedLinks.push(data.url);
                      } catch (err) {
                        alert("Lỗi upload hình: " + (err as any)?.message);
                      }
                    }
                    setForm(f => ({
                      ...f,
                      hinh: [...(f.hinh as string[] || []), ...uploadedLinks],
                    }));
                    // Reset input value để có thể upload lại cùng file nếu muốn
                    e.target.value = "";
                  }}
                />
                {/* Danh sách preview hình và input link */}
                {(form.hinh as string[] || []).length > 0 && (
                  <div className="flex flex-col gap-2 mb-2 max-h-64 overflow-y-auto pr-2">
