...
const ProductDetailPage = () => {
  ...
  // Đảm bảo selectedVariant luôn nằm trong visibleVariants
  useEffect(() => {
    if (!product) return;
    ...
    // Nếu selectedVariant không còn trong visibleVariants, chọn lại variant đầu tiên
    if (!selectedVariant || !visibleVariants.some(v => v._id === selectedVariant._id)) {
      let found = null;
      if (variantId) {
        found = visibleVariants.find((v) => v._id === variantId);
      }
      setSelectedVariant(found || visibleVariants[0]);
    }
    // eslint-disable-next-line
  }, [product, visibleVariants, variantId]);

  // Lấy các dung lượng/màu chỉ từ visibleVariants
  const storages = Array.from(new Set(visibleVariants.map(v => v.dung_luong) || []));
  const colors = Array.from(new Set(visibleVariants.map(v => v.mau) || []));

  // Lấy images an toàn, fallback về ảnh mặc định nếu không có variant hoặc product
  let images: string[] = ["/images/no-image.png"];
  if (selectedVariant && selectedVariant.hinh && Array.isArray(selectedVariant.hinh)) {
    images = selectedVariant.hinh;
  } else if (product && product.hinh) {
    if (Array.isArray(product.hinh)) images = product.hinh;
    else images = [product.hinh];
  }
  ...
  selectedVariant?.gia ??
  (product?.variants?.[0]?.gia ?? product?.Gia ?? 0);

  const price = selectedVariant?.gia ?? (product?.Gia ? product.Gia * (1 - (product.khuyen_mai || 0) / 100) : 0);
            ...
            <div className="flex gap-2">
              {storages.map((s, idx) => (
                <button
                  ...
                  onClick={() => {
                    const v = visibleVariants.find(v => v.dung_luong === s);
